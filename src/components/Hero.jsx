...
export default function Hero() {
  ...
  const suggestions = useMemo(() => {
    ...
    return hospitalNames
      .filter((hospitalName) =>
        hospitalName.toLowerCase().includes(normalizedQuery),
      )
      .slice(0, 5)
  }, [query])

  const handleFindHospitalsNavigation = (searchValue = '') => {
    const trimmedValue = searchValue.trim()

    if (trimmedValue) {
      navigate(`${FIND_HOSPITALS_PATH}?search=${encodeURIComponent(trimmedValue)}`)
      return
    }

    navigate(FIND_HOSPITALS_PATH)
  }

  const handleSearchInputKeyDown = (event) => {
    if (event.key === 'Enter') {
      event.preventDefault()
      handleFindHospitalsNavigation(query)
    }
  }

  return (
    <section className="relative w-full overflow-hidden border-b border-white/10 bg-[#020617] pb-16 pt-10 sm:pb-20 sm:pt-16 lg:pb-28 lg:pt-20">
      <div className="pointer-events-none absolute inset-x-0 -top-64 z-0 flex justify-center">
        <div className="h-72 w-full max-w-[36rem] rounded-full bg-[radial-gradient(circle_at_top,_rgba(34,197,94,0.35),_transparent_70%)] blur-3xl" />
      </div>

      <div className="relative z-10 mx-auto flex w-full max-w-7xl flex-col gap-12 px-4 sm:px-6 lg:flex-row lg:items-center lg:gap-16 lg:px-8">
        <div className="max-w-2xl">
          ...
          <div
            id="coverage"
            className="mt-8 grid grid-cols-2 gap-4 text-xs text-slate-300 sm:text-sm"
          >
            {[
        ...
        <div className="relative flex-1">
          <div className="relative mx-auto w-full max-w-xl rounded-3xl border border-emerald-300/20 bg-slate-950/60 p-6 shadow-[0_18px_60px_rgba(16,185,129,0.35)]">
            ...
            >
                        ...
                        {suggestions.map((hospitalName) => (
                          <button
                            ...
                            onClick={() => {
                              ...
                              handleFindHospitalsNavigation(hospitalName)
                            }}
                          >
                            {hospitalName}
                          </button>
              ...
              <button
                type="button"
                onClick={() => handleFindHospitalsNavigation(query)}
